...
LONG __stdcall CreateCrashHandler(EXCEPTION_POINTERS *pException) {
    ...
    if (DllHandle) {
        ...
        if (Dump) {
            ...
            // dmp文件的命名
            QString dumpDir = QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation);
            if (dumpDir.isEmpty()) {
                dumpDir = QDir::temp().absoluteFilePath("Throne");
            }
            QDir().mkpath(dumpDir);
            QString dumpText = QDir(dumpDir).absoluteFilePath("Dump_" + current_date + ".dmp");
            EXCEPTION_RECORD *record = pException->ExceptionRecord;
            QString errCode(QString::number(record->ExceptionCode, 16));
            QString errAddr(QString::number((uintptr_t) record->ExceptionAddress, 16));
            QString errFlag(QString::number(record->ExceptionFlags, 16));
            QString errPara(QString::number(record->NumberParameters, 16));
            const QString nativeDumpPath = QDir::toNativeSeparators(dumpText);
            HANDLE DumpHandle = CreateFile((LPCWSTR) nativeDumpPath.utf16(),
